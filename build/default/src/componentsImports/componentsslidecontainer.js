...
ns.slide.prototype.init=function(options){var self=this;altura=$(container+" "+items).find("li").height();cntli=$(container+" "+items).find("li").length;delta=100/cntli;ii=0;$(container+" "+items+" ul").css({visibility:"hidden"});$(container+" "+items).find("ul").css({width:100*cntli+"%"});$(container+" "+items).find("li").css({width:100/cntli+"%"});//console.dir($(container+" "+items).find("li").height());
createControlSelector(cntli);currentSelector(ii);if(0==ii)$(container+" "+btnleft).css({visibility:"hidden"});if(0<cntli)$(container+" "+btnright).css({visibility:"visible"});//********************************************************
...
var mytimer=setTimeout(function(){$(container+" "+items+" ul").css({visibility:"visible"})},250)}})(this.controls=this.controls||{});//this.components=components;
//console.dir(this.components);
//export {components}